...
file_list = list.files(PubMedDir, pattern= "PARTURITION")  # other pregnancy-related genes  ***
...
cleaned_abstracts = list() # simple cummulator 
stats_excl_ON = stats_excl_OFF = NULL # cummulator of cleaning summary stats
stats = NULL # cummulator of simple summary stats
...
for (exclusivity_pruning in c(TRUE,FALSE)) {
        for (phe in phes) {
               ...
               if (length(bad.lines)>0) raw.txt=raw.txt[-bad.lines]
                ...
                # clean-up
               if (sum(animal_test)>0) raw.txt=raw.txt[-which(animal_test)]
               ...
               #####################################################################################
                ...
                # final clean-up
               if (sum(disease_test)>0) raw.txt=raw.txt[-which(disease_test)]
...
cleaned_abstracts[["stats"]] = stats_excl_OFF

#  save what was generated (cleaned)
out_dir="~/Biostuff/MOBA_GESTAGE_GWAS/PREGNANCY_GENES/PubMed_2015Jun/PubMed_PRUNE/"
save(list=c("cleaned_abstracts"),file=paste(out_dir,"cleaned_abstracts_DELIVERY.RData",sep=""))

rm(list=ls())

# load what was generated (cleaned)
out_dir="~/Biostuff/MOBA_GESTAGE_GWAS/PREGNANCY_GENES/PubMed_2015Jun/PubMed_PRUNE/"
load(paste(out_dir,"cleaned_abstracts_DELIVERY.RData",sep=""))
