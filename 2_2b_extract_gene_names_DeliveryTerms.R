...
# extract PubMed genes that are related to DELIVERY (based on abstract mining)
# (not to confuse with script that searchers OBGYNtissue-related genes)
...
acronym_dir="~/Biostuff/MOBA_GESTAGE_GWAS/PREGNANCY_GENES/PubMed_2015Jul/"
...
for (phenotype in phenotypes) {
        for (translation in c("trn","unt")) {
                ...
                obj_name = paste(phenotype,translation,sep="_")
                print(obj_name)
                
                tempor = get("cleaned_abstracts")
                raw.txt = tempor[[phenotype]]; length(raw.txt); rm(tempor)
                
                # DECISION WHETHER TRANSLATOR should be used
                if (translation=="trn") {
                        transl_dir="~/Biostuff/MOBA_GESTAGE_GWAS/PREGNANCY_GENES/PubMed_2015Jul/"
        ...
        } # end of cycling through translation
...
result_dir = "~/Biostuff/MOBA_GESTAGE_GWAS/PREGNANCY_GENES/PubMed_2015Jul/WORK_FILES/" 
save(list=c("gene_lists"),file=paste(result_dir,"PubMed_extracted_genes_OBSTETRICS.RData",sep=""))
